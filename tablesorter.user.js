...
// ==UserScript==
// @name          TableSorter
// @namespace     http://codebox.org.uk/TableSorter
...
(function() {
    ...
    function processTable(tableElement) {
        ...
        function sortColByDate(col, ascending) {
            function makeNumFromText(txt) {
                return parseFloat(txt.replace(/,/g, '').replace(/[^0-9\.+\-]+/, ' '));
            }
            function makeDateFromText(txt){
                //return moment(txt, "DD MMM YYYY hh:mm A");
                return parseFloat(moment(txt).valueOf());
            }

            if (col<colCount){
                var multiplier = ascending ? 1 : -1;
                rowObjects.sort(function(a,b){
                    var aNum = makeDateFromText(a.getCellValue(col));
                    if (isNaN(aNum)){
                        var aNum = makeNumFromText(a.getCellValue(col));
                        if (isNaN(aNum)){
                         // Make non-numeric values go to the bottom of the list
                    ...
                    var bNum = makeDateFromText(b.getCellValue(col));
                    if (isNaN(bNum)){
                        var bNum = makeNumFromText(b.getCellValue(col));
                        if (isNaN(bNum)){
                         // Make non-numeric values go to the bottom of the list
        ...
        function setupSortRows() {
            ...
            var optionDateSortLink = document.createElement("A");
            optionDateSortLink.style.paddingLeft = '0.5em';
            optionDateSortLink.style.cursor = 'pointer';
            optionDateSortLink.innerHTML = '&#x21D5;Date';
            optionDateSortLink.title = 'Sort by date';
            optionDateSortLink.onclick = function (e) {
                setSortType(sortTypes.DATE);
            };
            sortTypes.DATE.link = optionDateSortLink;

            optionsTd.appendChild(optionsSpan);
            optionsTd.appendChild(optionAlphaSortLink);
            optionsTd.appendChild(optionNumSortLink);
            optionsTd.appendChild(optionDateSortLink);
